function padLeft (baseStr, n, str) {
  if (n < String(baseStr).length) {
    return baseStr.toString()
  } else {
    return Array(n - String(baseStr).length + 1).join(str || '0') + baseStr
  }
}
...
function generatePayNow (opts) {
  const p = [
    ...
    {
      id: '26',
      value: // ID 26: Merchant Account Info Template
    ...
    { id: '54', value: opts.amount }, // ID 54: Transaction Amount
    { id: '58', value: 'SG' }, // ID 58: 2-letter Country Code (SG)
    { id: '59', value: opts.entity }, // ID 59: Company Name
    { id: '60', value: 'Singapore' }, // ID 60: Merchant City
    {
      id: '62',
      value: [{ // ID 62: Additional data fields
  ...
  let str = p.reduce((final, current) => {
    ...
    if (current.value) {
      final += current.id + padLeft(current.value.length.toString(), 2) + current.value
    }
    return final
  }, '')
